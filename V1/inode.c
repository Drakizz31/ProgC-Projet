...
void AfficherInode(tInode inode) {
  ...
  printf("Date acces   : %s", ctime(&((*inode).dateDerAcces)));
  printf("Date modif fichier : %s", ctime(&((*inode).dateDerModif)));
  printf("Date modif inode   : %s", ctime(&((*inode).dateDerModifInode)));
}
